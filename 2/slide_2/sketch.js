...
function windowResized() {
	...
	radiations[0].p.set(width / 3, height / 2);
	radiations[1].p.set(2 * width / 3, height / 2);
...
class Wave{
	...
	draw(){
		...
		beginShape()
			for(let i=0;i<100;i++){
				let rr = map(noise(i,frameCount/500,this.randomId),0,1,0.9,1.1)*this.r
				let ang = i+ random()*0
...
function handleMousePress() {
  // Check if the mouse press is on any of the dots
  for (let i = 0; i < radiations.length; i++) {
    let d = dist(mouseX, mouseY, radiations[i].p.x, radiations[i].p.y);
    if (d < 10) {
      // If the press is on a dot, emit a wave
      radiations[i].emitWave();
    }
  }
}
